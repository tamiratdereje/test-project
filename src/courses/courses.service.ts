...
export class CoursesService {
  ...
  async getAllCourses(
  ...
  ): Promise<{ courses: Courses[]; count: number }> {
    ...
    const courses = await this.coursesModel
      .find({ ...JSON.parse(queryStr), ...searchQuery })
      .populate('instructor')
      .exec();
    const count = await this.coursesModel.countDocuments({
      ...JSON.parse(queryStr),
      ...searchQuery,
    });
  ...
  async updateCourse(id: string, courseInfo: Courses): Promise<Courses> {
    ...
    return await this.coursesModel
      .findByIdAndUpdate(id, updateObj, { returnDocument: 'after' })
      .populate('instructor');
  }
